...
- **[Home Connect](#home-connect)**
  ...
  - **[Home Connect API Errors](#home-connect-api-errors)**
    ...
    - [Why does my hood fail with `409 Conflict` and `Cooking.Common.Option.Hood.Boost`?](#why-does-my-hood-fail-with-409-conflict-and-cookingcommonoptionhoodboost)
  ...
  - **[Appliance Status and Connectivity](#appliance-status-and-connectivity)**
    ...
    - [Why does my refrigerator or freezer show as open in HomeKit even when it is closed?](#why-does-my-refrigerator-or-freezer-show-as-open-in-homekit-even-when-it-is-closed)
...
#### Why does my hood fail with `409 Conflict` and `Cooking.Common.Option.Hood.Boost`?

<!-- INCLUDES: issue-388-a426 issue-392-d5d9 -->
This error, typically logged as `[409 Conflict] Unable to find default value for default option: Cooking.Common.Option.Hood.Boost [SDK.Error.MissingOptionValue]`, indicates a Home Connect API server-side regression. It occurs when attempting to start a venting program or change the fan speed on certain hood models.
...
2. Report the issue directly to [Home Connect developer support](https://developer.home-connect.com/support/contact). Provide your account email address, the appliance `haID` (available in the Homebridge logs), and the specific error message including the `Cooking.Common.Option.Hood.Boost` key.
...
The plugin performs strict validation on data from the Home Connect API to ensure reliability. Because the API often deviates from its official documentation, or because new appliance models (such as the Spotless cleaning robot series) and firmware introduce undocumented features, the plugin includes a diagnostic mechanism to identify identifiers it does not yet recognise. When the plugin encounters these values or `(unrecognised)` tags, it generates a technical diagnostic block in the log, formatted as TypeScript code and delimited by rows of `=` characters. This helps the maintainer update the plugin's internal schema and map features to HomeKit services.
...
3. **Provide context**: Note which setting you were changing in the official Home Connect app when the log entry appeared.
4. **Provide the snippet**: Paste the entire technical diagnostic block from the log (including the `=` separators) into the **Log File** field of the issue template.
...
1. **Expose individual door services**: In the appliance-specific settings within the plugin configuration (available in `v1.9.1` and later), disable the generic `Door` feature and instead enable the specific compartment door features. These specific keys (such as `Freezer Door`, `Refrigerator Door`, `Chiller Left Door`, `Chiller Right Door`, or `Flex Compartment Door`) often report the correct state even when the aggregate status is broken.
2. **Enable debug logging**: Use the **Log Debug as Info** option to see the raw values being returned by the Home Connect API. This confirms if the plugin is receiving `BSH.Common.EnumType.DoorState.Open` or `Refrigeration.Common.EnumType.Door.States.Open` from the API server while the door is physically closed.
